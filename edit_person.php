<?php
session_start(); // important function to allow session variables
$connectionServerName = "localhost:3306";
$connectionUsername = "admin123";
$connectionPassword = "Password123";
$connection = new PDO("mysql:host=$connectionServerName;dbname=accounts", $connectionUsername, $connectionPassword);
$connection->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
//include 'checklogin.php';
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>Document</title>

    <style>
    body{
        font-family:tahoma;
        background-color:#eee;

    }
    input[type=submit] {
        font-size:110%;
        background-color:green;
        color:white;
        border-radius: 4px;
    }
    select {
        width: 100%;
        padding: 16px 20px;
        border: none;
        border-radius: 4px;
        background-color: #fff;
        font-size:110%;
        margin:5px 0 5px 0;
        border:1px solid #333;
        }
</style>
</head>
<body>
    <?php

        //database connection variables
        //include 'dbconnect.php';

        try {
            if($_SERVER['REQUEST_METHOD'] == 'POST') //has the person been updated
            {

                $conn = new PDO("mysql:host=$connectionServerName;dbname=accounts", $connectionUsername, $connectionPassword);
                // set the PDO error mode to exception
                $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

                $id = $_POST['id']; //the ID of the order we wanted to edit from the hidden form field
                //all other information comes from the form entries
                $faultname = $_POST['faultname'];
                $faultdesc = $_POST['faultdesc'];
                $faultlocation = $_POST['faultlocation'];
                $user = $_POST['user'];
                $time = $_POST['time'];
                $faultstatus = $_POST['faultstatus'];
                $jobcomp = $_POST['jobcomp'];



                //preparing an sql to update the t_people table
                $sql = $conn->prepare("UPDATE reporttable SET faultname = :faultname, faultdesc = :faultdesc, faultlocation = :faultlocation, user = :user, time = :time, faultstatus = :faultstatus, jobcomp = :jobcomp WHERE id = :id");
                $sql -> bindParam(':id', $id);
                $sql -> bindParam(':faultname', $faultname);
                $sql -> bindParam(':faultdesc', $faultdesc);
                $sql -> bindParam(':faultlocation', $faultlocation);
                $sql -> bindParam(':user', $user);
                $sql -> bindParam(':time', $time);
                $sql -> bindParam(':faultstatus', $faultstatus);
                $sql -> bindParam(':jobcomp', $jobcomp);
                $sql -> bindParam(':id', $id);

                $sql -> execute(); //execute the statement

                header("Location: edits.php");
                exit();
            }
            else{
                $conn = new PDO("mysql:host=$connectionServerName;dbname=accounts", $connectionUsername, $connectionPassword);
                // set the PDO error mode to exception
                $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

                $id = $_GET['id']; //the ID of the person we want to edit from the URL

                $sql = $conn->prepare("SELECT * FROM reporttable WHERE id = :id");
                $sql -> bindParam(':id', $id);

                $sql -> execute(); //execute the statement



                $row = $sql->fetch();
                $faultname = $row['faultname'];
                $faultdesc = $row['faultdesc'];
                $faultlocation = $row['faultlocation'];
                $user = $row['user'];
                $time = $row['time'];
                $faultstatus = $row['faultstatus'];
                $jobcomp = $row['jobcomp'];

               //Use an include to include the edit person form and pre-fill in the form entries with existing data
                include "edit_person_form.php";


            }
        }
        catch(PDOException $e)
            {
            echo $e->getMessage(); //If we are not successful in connecting or running the query we will see an error
            }
        ?>


</body>
</html>
